###########Univariate emulator for floc data
library(doParallel)
library(abind)
setwd("./Univariate_Floc")
dirname=getwd()
nout=4
#########################################################################
##Histograms in Figure 3
bb3=read.csv("All_Floc.csv")##read in all floc data
...
###########################################EMULATOR FITTING
f=list(
~1 + y1 + nh4 + no3 + no2 + o2 + s + diffT + Ds + Dno3 + Dno2 + 
    ...
    KsHET
,
~1 + y2 + nh4 + no3 + no2 + o2 + s + diffT + Ds + Dno3 + Dno2 + 
    Dnh4 + Do2 + Y1 + YEPS + YNOB + YAOB + YHET + bEPS + bNOB + 
    bAOB + bHET + etaHET + MumNOB + MumAOB + MumHET + Ko2NOB + 
    Kno2NOB + Ko2AOB + Knh4AOB + Kno3HET + Kno2HET + Ko2HET + 
    KsHET
,
~1 + y3 + nh4 + no3 + no2 + o2 + s + diffT + Ds + Dno3 + Dno2 + 
    Dnh4 + Do2 + Y1 + YEPS + YNOB + YAOB + YHET + bEPS + bNOB + 
    bAOB + bHET + etaHET + MumNOB + MumAOB + MumHET + Ko2NOB + 
    Kno2NOB + Ko2AOB + Knh4AOB + Kno3HET + Kno2HET + Ko2HET + 
    KsHET
,
~1 + y4 + nh4 + no3 + no2 + o2 + s + diffT + Ds + Dno3 + Dno2 + 
    Dnh4 + Do2 + Y1 + YEPS + YNOB + YAOB + YHET + bEPS + bNOB + 
    bAOB + bHET + etaHET + MumNOB + MumAOB + MumHET + Ko2NOB + 
    Kno2NOB + Ko2AOB + Knh4AOB + Kno3HET + Kno2HET + Ko2HET + 
    KsHET)
...
library(DiceKriging)
km(formula=f[[i]],X[[i]],response=Y[,i],nugget=nug[i],covtype="exp")
}
dirname=getwd()
...
nvar=27+5;nout=4
nam=c("KsHET","Ko2HET","Kno2HET","Kno3HET","Knh4AOB","Ko2AOB","Kno2NOB","Ko2NOB","MumHET","MumAOB","MumNOB","etaHET","bHET","bAOB","bNOB","bEPS","YHET","YAOB","YNOB","YEPS","Y1","Do2","Dnh4","Dno2","Dno3","Ds","diffT","s","o2","no2","no3","nh4","y1","y2","y3","y4")
X1 <- data.frame(matrix(runif((nvar+1) * n), nrow = n))
X2 <- data.frame(matrix(runif((nvar+1) * n), nrow = n))
library(sensitivity)
sens=list()
for(i in 1:nout){
...
val[[k]]=read.csv(paste(sub("k",k,paste("Univariate_Floc","Y_k.csv",sep="/")),sep="/"))
}
ini=as.matrix(read.csv("Univariate_Floc/XX_ini.csv"))##matrix of initial parameters
setwd(dirname)
...
X[[i]]=vv
m1=predict(modd[[i]],newdata=X[[i]],type="UK",se.compute=TRUE)
