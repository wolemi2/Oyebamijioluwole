...
MatrixXf H = X.array().log();//log-tranformed training inputs
MatrixXf H0 = newdata.array().log();//log of test input
//H=X
int m=H.cols();
int n=Y.rows();
int n2=H0.rows();
...
MatrixXf iOmega = (H.transpose()*Ainv*H).inverse();//can be precomputed in R
MatrixXf betahat= iOmega*(H.transpose()*Ainv*Y);// can be precomputed in R
