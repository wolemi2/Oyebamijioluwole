...
mat=matrix(0,nrow=259200,ncol=12)
s3a=as.matrix(s3a)
mat[,1:2]=s3a
mat[r3a,3:7]=mydata[,1:5]
mat[r3a,8:12]=mydata[,6:10]
colnames(mat)=c("Lon","Lat","cereal","rice","maize","oil","grd","ini_cereal","ini_rice","ini_maize","ini_oil","ini_grd")
 mat[r3a,3:12]=mat[r3a,3:12]*lpj.area
 mat=as.data.frame(mat)
coordinates(mat) = c("Lon", "Lat")
gridded(mat) <- TRUE
mat1 = as(mat, "SpatialGridDataFrame")
crp1=matrix(0,nrow=186,ncol=10)
for(t in 1:10){
crp1[t]=aggregateHalfDegreeGridToCountries(mat1[t], aggregateOption = "sum")[2]
}
crp <- data.frame(matrix(unlist(crp1[1:10]), nrow=186))
colnames(crp)=c("cereal","rice","maize","oil_max","grd","ini_cereal","ini_rice","ini_maize","ini_oil_max","ini_grd")
