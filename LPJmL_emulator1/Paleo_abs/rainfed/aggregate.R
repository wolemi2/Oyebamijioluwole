...
mat=matrix(0,nrow=259200,ncol=5)
s3a=as.matrix(s3a)
mat[,1:2]=s3a
mat[r3a,3:5]=mydata[,1:3]
#mat[r3a,8:12]=mydata[,6:10]
colnames(mat)=c("Lon","Lat","cereal","rice","maize")
 mat[r3a,3:5]=mat[r3a,3:5]*lpj.area
 mat=as.data.frame(mat)
coordinates(mat) = c("Lon", "Lat")
gridded(mat) <- TRUE
mat1 = as(mat, "SpatialGridDataFrame")
crp1=matrix(0,nrow=186,ncol=3)
for(t in 1:3){
crp1[t]=aggregateHalfDegreeGridToCountries(mat1[t], aggregateOption = "sum")[2]
}
crp <- data.frame(matrix(unlist(crp1[1:3]), nrow=186))
colnames(crp)=c("cereal","rice","maize")
res=cbind(Country2,crp)
}}
stopCluster(cl)
dirname <- paste(getwd(),"output_result",sep="/")
if(j==0) {
write.csv(result_model,file=paste(dirname,sub("v3",v3,(sub("j",1,(sub("j",j,"crop_ALL_10_v3_result.csv"))))),sep="/"),row.names=FALSE)
