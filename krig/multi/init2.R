...
param=c(mod@covar@range.val,mod@covar@sd2)
#if (length(parinit)>0) {
...
aux <- covMat1Mat2(mod@covar,m1@X,m2@X)
CC <- aux
#allpar=list(matrixinit,C)
#return(allpar)
#}
###################################
#beta=trend.estimate$coef
#fninit <- apply(matrixinit,2,LL2,mod)
...
fninit=NULL
parinit=mean(varinit.sim)  #rowMeans(matrixinit)  	
lp <- length(parinit)
#covinit <- list()
pari <- as.numeric(parinit)
#covinit[[1]] <- vec1(mod@covar, pari[1:(lp-1)])
covinit <- pari[lp]
allpar=list(par = parinit, value = fninit, 
cov = covinit,lower =(varinit.lower), upper = (varinit.upper),CC=CC)
return(allpar)
}
###########
